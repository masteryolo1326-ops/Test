...
/* =========================================================
   SUPABASE: URL y ANON KEY (forzadas para evitar desajustes)
   ========================================================= */
...
/* =========================================================
   REDIRECT para recuperación de contraseña (Auth)
   - Usa la ruta actual (GH Pages): https://usuario.github.io/tu-repo/...
   - Asegúrate de permitir esta URL en Supabase → Auth → URL Configuration
   ========================================================= */
const PASSWORD_REDIRECT_URL = (() => {
  try {
    // Devuelve la URL de la página actual (sin query ni hash)
...
/* =========================================================
   APP DE VOTACIÓN
   - Misma lógica que tu script original, externalizada a app.js
   ========================================================= */
class VotingApp {
  constructor() {
  ...
  async handleLogin() {
    ...
    try {
      ...
      // postLoginBoot se disparará por onAuthStateChange
    } catch (e) {
      console.error("Auth error:", e);
      this.toast(e?.message || "Error de autenticación", "error");
    }
  }
  ...
  setupEventListeners() {
    ...
    if (resetBtn) {
      resetBtn.addEventListener("click", () => this.resetVotingLocal());
    }
  ...
  setupSlider() {
    ...
    if (!sliderButton || !sliderTrack || !sliderGlow || !sliderText || !sliderSuccess) return;
    ...
    const reset = () => {
      ...
      sliderGlow.style.transition = "opacity .4s ease-out";
      sliderGlow.style.opacity = "0";
      sliderText.style.transition = "opacity .4s ease-out";
      sliderText.style.opacity = "1";
      setTimeout(() => {
        currentX = 0;
        sliderButton.style.transition = "";
        sliderGlow.style.transition = "";
    ...
    const complete = () => {
      isCompleted = true;
      sliderTrack.style.transition = "opacity .5s ease-out";
      sliderTrack.style.opacity = "0";
      setTimeout(() => {
        sliderTrack.style.display = "none";
        sliderSuccess.style.display = "flex";
        sliderSuccess.style.opacity = "0";
        sliderSuccess.style.transition = "opacity .5s ease-in";
        setTimeout(() => {
          sliderSuccess.style.opacity = "1";
        }, 50);
      }, 500);
      setTimeout(() => {
        this.animateToLogin();
    ...
    sliderButton.addEventListener("mousedown", (e) => {
      ...
      sliderGlow.style.transition = "opacity .2s ease";
    });
  ...
  animateToLogin() {
    ...
    if (mainContent) mainContent.classList.add("slide-out-left");
    setTimeout(() => {
      this.showPage("login");
  ...
  animateToHome() {
    ...
    loginHeader?.classList.add("slide-out-up");
    ...
    setTimeout(() => {
      ...
      loginHeader?.classList.remove("slide-in-down", "slide-out-up");
      loginCard?.classList.remove("form-appear", "slide-out-left");
      mainTitle?.classList.add("title-return");
      setTimeout(() => {
        document.getElementById("votingSystem")?.classList.remove("voting-exit", "voting-appear");
        mainContent?.classList.remove("home-return");
        mainTitle?.classList.remove("title-exit", "title-return");
  ...
  animateToVoting() {
    ...
    const mainTitle = document.getElementById("mainTitle");
    mainTitle?.classList.add("title-exit");
    loginPage?.classList.add("login-to-voting");
    setTimeout(() => {
  ...
  logout() {
    ...
    setTimeout(() => {
      ...
      const mainContent = document.getElementById("mainContent");
      const mainTitle = document.getElementById("mainTitle");
      mainContent?.classList.add("home-return");
      mainTitle?.classList.add("title-return");
      setTimeout(() => {
  ...
  nextSlide() {
    ...
    const el = document.getElementById("carousel3d");
    if (el) el.style.transform = `rotateY(${this.totalRotation}deg)`;
    this.updateDots();
    if (!this.hasVoted) setTimeout(() => {
      this.userInteracting = false;
      this.startAutoCarousel();
    }, 3000);
  }
  ...
  updateDots() {
    ...
    dotsContainer.innerHTML = this.candidates
      .map((_, i) => `
        <button class="w-4 h-4 rounded-full transition-all duration-300 ${i === this.currentSlide ? "bg-white scale-125" : "bg-white/40"}"
          onclick="votingApp.goToSlide(${i})"></button>
      `)
      .join("");
  }
  ...
  renderCandidates() {
    ...
    const dotsContainer = document.getElementById("dotsContainer");
    if (!carousel3d || !dotsContainer) return;

    const angleStep = 360 / this.candidates.length;
    const radius = 220;

    carousel3d.innerHTML = this.candidates
      .map((candidate, index) => {
        const angle = index * angleStep;
        const initials = candidate.name.split(" ").map(n => n[0]).join("");
        const disabled = this.hasVoted ? "disabled" : "";
        const disabledCls = this.hasVoted ? "opacity-50 cursor-not-allowed" : "";
        return `
          <div class="carousel-3d-item"
               style="position:absolute; left:50%; top:50%; width:220px; height:280px; margin-left:-110px; margin-top:-140px; transform: rotateY(${angle}deg) translateZ(${radius}px); transform-style:preserve-3d; backface-visibility:visible;">
            <div class="candidate-card rounded-2xl p-4 border border-gray-300 shadow-2xl relative overflow-hidden h-full transition-all duration-300"
              ...
              <button class="modern-btn w-full flex items-center justify-center space-x-1 group/btn relative z-10 text-xs py-2 transition-all duration-300 ${disabledCls}"
                      data-candidate-id="${candidate.id}" ${disabled}>
                ${
                  this.hasVoted
                    ? `
  ...
  openVoteModal(candidateId) {
    ...
    const selEl = document.getElementById("selectedCandidate");
    if (selEl) selEl.textContent = candidate.name;
  ...
  async confirmVote() {
    ...
    if (!this.userId) {
      this.toast("Debes iniciar sesión con tu correo UNISON", "error");
      return;
    }
    ...
    try {
      ...
      this.renderResults();
      this.updateTotalVotes();
      const name = this.candidates.find((c) => c.id === this.selectedCandidateId)?.name || "candidato";
      this.showVoteConfirmation(name);
    } catch (err) {
  ...
  async syncCountsFromDB() {
    ...
    if (error) {
      console.error("syncCountsFromDB error:", error);
  ...
  async checkIfUserHasVoted() {
    ...
    if (error && error.code !== "PGRST116") {
      console.error("checkIfUserHasVoted error:", error);
  ...
  renderResults() {
    ...
    container.innerHTML = this.candidates
      .map((candidate, index) => {
        const percentage = totalVotes > 0 ? ((candidate.votes / totalVotes) * 100).toFixed(1) : 0;
        const isLeading =
          totalVotes > 0 && candidate.votes === Math.max(...this.candidates.map((c) => c.votes));
        const colors = [
  ...
  getTotalVotes() {
    return this.candidates.reduce((t, c) => t + (c.votes || 0), 0);
  }

  updateTotalVotes() {
    const el = document.getElementById("totalVotes");
    if (el) el.textContent = this.getTotalVotes();
  }
  ...
  showVoteConfirmation(name) {
    this.toast(`¡Voto registrado para ${name}!`, "success");
  }
}

/* =========================================================
   Bootstrap: crea la app cuando el DOM está listo
   ========================================================= */
(function boot() {
